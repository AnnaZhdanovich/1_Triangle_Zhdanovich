...
public class ActionTriangle {		
	...
	public boolean checkRectangular(Triangle t) {		
	    boolean result =((Math.sqrt(createSide(t.getA(),t.getB()))== Math.sqrt(createSide(t.getB(),t.getC()))+ Math.sqrt(createSide(t.getC(),t.getA()))) &&(Math.sqrt(createSide(t.getB(),t.getC()))==Math.sqrt(createSide(t.getA(), t.getB()))+Math.sqrt(createSide(t.getC(), t.getA())))&&(Math.sqrt(createSide(t.getC(), t.getA()))==Math.sqrt(createSide(t.getA(), t.getB()))+Math.sqrt(createSide(t.getB(), t.getC())) && (this.area(t)!=0)));
		return result;
	}
	
	private double createSide(Point a, Point b) {
